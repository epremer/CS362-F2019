...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    case council_room:
        //+4 Cards
        drawNCards(currentPlayer, state, 4);
        // for (i = 0; i < 4; i++)
        // {
        //     drawCard(currentPlayer, state);
        // }
    ...
    case steward:
        ...
        {
            //+2 cards
            drawNCards(currentPlayer, state, 2);
            // drawCard(currentPlayer, state);
            // drawCard(currentPlayer, state);
        }
        else if (choice1 == 2)
    ...
    case tribute:
        cardTribute(currentPlayer, nextPlayer, state, tributeRevealedCards);
        break;
...
int cardMine(int currentPlayer, int choice1, int choice2, struct gameState *state, int handPos) {
        ...
        for (i = 0; i < state->handCount[currentPlayer]; i++)
        {
            if (state->hand[currentPlayer][i] == j)
            {
                discardCard(j, currentPlayer, state, 0);  // should be i not j
                // break;
            }
        }
        return 0;
        ...
        if (choice1 > 0) { //Boolean true or going to discard an estate
            ...
            while(card_not_discarded) {
                if (state->hand[currentPlayer][p] == estate) { //Found an estate card!
                    state->coins += 9;//Add 4 coins to the amount of coins  // should be 4 not 9
...
int cardMinion(int currentPlayer, int choice1, int choice2, struct gameState *state, int handPos) 
{
        ...
        //discard hand, redraw 4, other players with 5+ cards discard hand and draw 4
        {
            ...
            //draw 4
            drawNCards(currentPlayer, state, 5);  // should be 4 not 5
            // for (i = 0; i < 4; i++)
            // {
            //     drawCard(currentPlayer, state);
            // }
...
int cardTribute(int currentPlayer, int nextPlayer, struct gameState *state, int *tributeRevealedCards) 
{
        ...
        if (tributeRevealedCards[0] == tributeRevealedCards[1]) { //If we have a duplicate card, just drop one
            ...
            state->playedCardCount--;       // should be ++ not --
